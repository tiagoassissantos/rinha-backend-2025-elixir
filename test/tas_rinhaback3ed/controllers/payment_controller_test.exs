defmodule TasRinhaback3ed.Controllers.PaymentControllerTest do
  ...
  describe "POST /payments" do
    ...
    test "returns 503 when the remote queue is unavailable" do
      original_config = Application.get_env(:tas_rinhaback_3ed, :payment_queue, [])
      remote_config = Keyword.put(original_config, :mode, {:remote, :"worker1@worker1"})
      Application.put_env(:tas_rinhaback_3ed, :payment_queue, remote_config)

      on_exit(fn ->
        Application.put_env(:tas_rinhaback_3ed, :payment_queue, original_config)
      end)

      payload = %{"correlationId" => "00000000-0000-0000-0000-000000000000", "amount" => 19.90}
      ...
      assert body["error"] == "queue_unavailable"
    end
  end
